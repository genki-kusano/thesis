...
def parameter_birth_death_pers(list_pd):
    ...
    for k in range(num_pd):
        ...
        vec_med_p[k] = np.median(vec_pers)
        vec_max_p[k] = np.max(vec_pers)
    return np.min(vec_b), np.max(vec_d), np.median(vec_med_p), np.max(vec_max_p)
...
def plot_gram(mat_gram, name_title="", show=True):
    plt.figure()
